...
function unlock(model, resourceId, clientId, eventId) {
    return __awaiter(this, void 0, void 0, function* () {
        if (RedisService.redis) {
            const key = (0, getRedisResourceLockKey_1.default)(model, resourceId);
            const value = yield RedisService.get(key);
            if (!value || value.locker !== clientId)
                return false;
            yield RedisService.del(key);
            yield RedisService.hdel((0, getRedisEventResourceLockKey_1.default)(eventId), key);
            return true;
        }
        else {
            ...
            if (!lock || lock.locker !== clientId)
                return false;
            lock.status = _Types_1.ResourceLockStatus.UNLOCKED;
            yield lock.save();
            return true;
        }
    });
